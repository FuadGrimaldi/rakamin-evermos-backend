...
func (h *TokoHandler) UpdateToko(c *fiber.Ctx) error {
	...
	// Upload langsung ke Cloudinary pakai stream
	urlFoto, err := util.UploadToCloudinary(c.Context(), file, fileHeader.Filename)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"message": "Failed to upload to Cloudinary", "error": err.Error()})
	}

	// Update URL Foto ke Database
	request.UrlFoto = &urlFoto
	err = h.tokoService.UpdateToko(c.Context(), id, &request)
	if err != nil {
